...
class OperatingSystem:
    ...
    def sys_fork(self):
        ...
        if all(
            not f.frame.f_locals["fork_child"]
            for f in inspect.stack()
            if f.function == "_step"
        ):  # this is parent; do fork
    ...
    def sys_bread(self, key):
        ...
        return {
            "bread": (
                lambda: storage.buf.get(
                    key,  # always try to read from buffer first
    ...
    def sys_sync(self):
        ...
        def do_sync():
            ...
            self._storage = Storage(
                persist=store.persist | store.buf, buf={}  # write back
            )
    ...
    def sys_crash(self):
        ...
        crash_sites = (
            lambda subset=subset: setattr(
                self,
                "_storage",
                Storage(  # persist only writes in the subset
                    persist=persist | dict(compress(btrace, subset)), buf={}
                ),
            )
            for subset in product(  # Mosaic allows persisting any subset of  # pending blocks in the buffer
                *([(0, 1)] * len(btrace))
            )
        )
    ...
    def state_dump(self) -> dict:
        ...
        return copy.deepcopy(os_state) | dict(  # freeze the runtime state
            hashcode=f"{h:016x}"
        )
...
class Mosaic:
    ...
    class Transformer(ast.NodeTransformer):
        def visit_Call(self, node):
            ...
            if (
                isinstance(node.func, ast.Name) and node.func.id in SYSCALLS
            ):  # rewrite system calls
                return ast.Yield(
                    ast.Tuple(  #   -> yield ('sys_xxx', args)
                        elts=[
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "source", help="application code (.py) to be checked; must have a main()"
    )
